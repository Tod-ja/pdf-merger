...
def label_files():
    try:
        ...
        for file, label in zip(files, labels):
            ...
            category_files[label].append(file)
            ...
            for file in files_in_category:
                ...
                for page_num, page in enumerate(pages):
                    if page_num == 0 and start_number is not None:
                        label_with_number = f"{label}{start_number}" if label != " " else f"{start_number}"
                        start_number += 1  # Increment start number for the next file
